// prototype of function to calculate size of Z80N opcode
//
...
byte opcodeSz(byte *b) {
    if (0xCD == (b[0] & 0xCF)) {                // call ** / ED / IXY
        ...
        if (0xED == b[0]) {                     // extended instructions
            ...
            if (0x92 == b[1] || 0x27 == b[1]) return 3;     //Z80N nextreg imm8,A : test imm8
            if (0x34 <= b[1] && b[1] <= 0x36) return 4;     //Z80N add r16,imm16
            if (0x8A == b[1] || 0x91 == b[1]) return 4;     //Z80N push imm16 : nextreg imm8,imm8
